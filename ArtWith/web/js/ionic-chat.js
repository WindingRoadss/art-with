  ...
  function($scope, $rootScope, $state, $stateParams, MockService,
    ...
    $scope.$on('$ionicView.enter', function() {
      ...
      messageCheckTimer = $interval(function() {
        // here you could check for new messages if your app doesn't use push
		// notifications or user disabled them
      }, 20000);
    });
    ...
    function getMessages() {
      // the service is mock but you would probably pass the toUser's GUID
		// here
    ...
    $scope.sendMessage = function(sendMessageForm) {
      ...
      // if you do a web service call this will be needed as well as before
		// the viewScroll calls
      // you can't see the effect of this in the browser it needs to be used
		// on a real device
      // for some reason the one time blur event is not firing in the browser
		// but does on devices
      keepKeyboardOpen();
    ...
    // this keeps the keyboard open on a device only after sending a message, it
	// is non obtrusive
  ...
  function($http, $q) {
    ...
    me.getUserMessages = function(d) {
      /*
		 ...
		 * return $http.jsonp(endpoint).then(function(response) { return
		 * response.data; }, function(err) { console.log('get user messages
...
/*
 * angular-elastic v2.4.2 (c) 2014 Monospaced http://monospaced.com License: MIT
 */
      ...
      return {
        ...
        link: function(scope, element, attrs, ngModel) {
          ...
          function adjust() {
            ...
            // active flag prevents actions in function from calling adjust
			// again
            if (!active) {
              active = true;

              mirror.value = ta.value + append; // optional whitespace to
												// improve animation
              ...
              // ensure getComputedStyle has returned a readable 'used value'
				// pixel width
              ...
              scope.$emit('taResize', $ta); // listen to this in the
											// UserMessagesCtrl
...
// autolinker
/*
 * ! Autolinker.js 0.15.0
 * 
 * Copyright(c) 2014 Gregory Jacobs <greg@greg-jacobs.com> MIT Licensed.
